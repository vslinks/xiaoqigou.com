        ...
        <form method="post" action="<?php echo U('');?>" enctype="multipart/form-data" >
            <table cellspacing="1" cellpadding="3" width="100%">
                ...
                <tr>
                    ...
                    <td>
                       <select name="article_category_id" value="<?php echo ((isset($row["article_category_id"]) && ($row["article_category_id"] !== ""))?($row["article_category_id"]):0); ?>" class="article_category_id">
                           <option value="0">请选择...</option>
                           <?php if(is_array($article_cate_list)): $i = 0; $__LIST__ = $article_cate_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>"><?php echo ($val); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                       </select>
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    <td class="label">文章详细内容</td>
                    <td>
                        <textarea  name="content" cols="60" rows="10"><?php echo ($article_content_list[$row['id']]); ?></textarea>
                    </td>
                </tr>
                <tr>
...
<!--下面这个是为 js  预留的位置-->
    ...
    <script type="text/javascript">
        $(function(){
            ...
            $('.article_category_id').val([<?php echo ((isset($row["article_category_id"]) && ($row["article_category_id"] !== ""))?($row["article_category_id"]):0); ?>])
